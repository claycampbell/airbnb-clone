...
const Home = ({ toggle, setToggle }) => {
  ...
  const cities = ['Los Angeles', 'Chicago', 'Seattle', 'New York', 'Boston', 'Austin'];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentCity(cities[(cities.indexOf(currentCity) + 1) % cities.length]);
    }, 4000);
      ...
      <nav className="bg-white w-full navBar flex relative justify-between items-center mx-auto px-8 h-20">
        <div className="flex justify-center items-center text-left">
          <div className="flex items-center">
            <h1 style={{ fontFamily: 'Figtree', fontSize: '24px', color: 'navy blue' }}>{currentCity}</h1>
           
              <div className="hidden md:block">
                <img src={logo} alt="Logo" style={{ display: 'block', width: '15%', height: '15%' }} />
              </div>
            ...
            <h1 style={{ fontFamily: 'Figtree', fontSize: '24px', color: 'navy blue' }}>Welcomes the World</h1></div>
